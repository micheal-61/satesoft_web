...
function Footer() {
  ...
  useEffect(() => {
    const fetchFooterData = async () => {
      try {
        const [contactsRes, privacyRes, agreementRes, servicesRes] =
          await Promise.all([
            fetch("/api/contacts")
              .then((r) => (r.ok ? r.json() : []))
              .catch(() => []),

            fetch("/api/privacy-policies")
              .then((r) => (r.ok ? r.json() : []))
              .catch(() => []),

            fetch("/api/service-agreements")
              .then((r) => (r.ok ? r.json() : []))
              .catch(() => []),
        ...
        const socials = contactsRes.filter(
          (c) => c.category === "social_media"
        );

        setSocialLinks(socials);
        setPrivacyPolicy(privacyRes.length > 0 ? privacyRes[0] : null);
        setServiceAgreement(
          agreementRes.length > 0 ? agreementRes[0] : null
        );
  ...
  return (
    <div>
      ...
      {/* MAIN FOOTER */}
      <div className="bg-[#f5f5f5] pt-8 pb-6 text-slate-600">
        <div className="container mx-auto px-8 md:px-12 lg:px-20">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6 mb-8">
            ...
            <div className="lg:col-span-4">
              ...
              <p className="mt-3 mb-4 leading-relaxed text-[#72bf24] text-[14px]">
                Empowering businesses across Africa with innovative cloud
                solutions, intelligent software, and actionable data
                analytics.
              </p>
                ...
                {loading ? (
                  <div className="flex gap-2">
                    {[1, 2, 3, 4].map((i) => (
                      <div
                        key={i}
                        className="w-9 h-9 rounded-full bg-slate-300 animate-pulse"
                      ></div>
                    ))}
                  </div>
                ) : (
            ...
            <div className="lg:col-span-3">
              ...
              <ul className="space-y-2">
                {loading ? (
                  <li className="text-[13px] text-text/40">
                    Loading services...
                  </li>
                ) : services.length > 0 ? (
            ...
            <div className="lg:col-span-3">
              ...
              <p className="mb-4 text-[#72bf24] text-[14px]">
                Subscribe to get the latest news, updates, and product
                insights from Satesoft.
              </p>
          ...
          <div className="border-t border-slate-300 pt-4 flex flex-col md:flex-row justify-between items-center gap-3">
            <p className="mb-0 text-[12px] font-normal">
              © Copyright 2026 By{" "}
              <span className="text-[#72bf24] font-semibold">
                Satesoft
              </span>
              . All rights reserved.
            </p>
